...
direct_lookup (struct inode *inode, size_t block_idx)
{
  ...
  block_sector_t inode_ofs = byte_ofs_inode_sector (inode->inumber);
  lock_acquire (&GENGAR);
...
inode_append_sector (struct inode *inode)
{
  ...
  size_t ofs = inode_ofs * sizeof (struct disk_inode);
  struct cache_block *cached_inode_sector; 
  struct disk_inode *inode_disk;
...
inode_open (block_sector_t inumber)
{
  ...
  struct cache_block *cached_inode_sector; 
  struct disk_inode *inode_disk;
  size_t inode_ofs = byte_ofs_inode_sector (inumber);
  lock_acquire (&GENGAR);
  if ((cached_inode_sector = cache_find_sector (sector)) == NULL)
    cached_inode_sector = cache_fetch (sector, INODE_DATA);
